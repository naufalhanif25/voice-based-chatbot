...
5. **Modern Multi-Turn History Management:** Multi-turn context is preserved using a standard Pydantic `TypeAdapter` that serializes and deserializes the conversation history into `chat_history.json`. It aligns fully with OpenAI-style structural conventions (`{"role": "user | assistant", "content": "..."}`).
